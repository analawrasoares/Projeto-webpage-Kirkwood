CREATE DATABASE  IF NOT EXISTS `ap` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci */;
USE `ap`;
...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `tr_invoices_after_update` AFTER UPDATE ON `invoices` FOR EACH ROW -- It is true for every row
    BEGIN
		-- Define the fields being inserted into
        ...
        -- List of value that we are going to insert
			OLD.vendor_id
            , OLD.invoice_number
            , OLD.invoice_total
            , 'UPDATE'         -- User UPDATE as a action
            , NOW()            -- Use new() time as action date
            , CURRENT_USER()
        );
...
INSERT INTO `invoices_audit` VALUES (123,'963253246',129.00,'UPDATE','2019-04-16 16:09:58','root@localhost'),(121,'97/486',953.10,'UPDATE','2019-04-16 16:09:58','root@localhost');
...
INSERT INTO `terms` VALUES (1,'Net due 10 days',10),(2,'Net due 20 days',20),(3,'Net due 30 days',30),(4,'Net due 60 days',60),(5,'Net due 90 days',90),(6,'Due 90 Days',90),(7,'',90);
...
--
-- Final view structure for view `vw_vendors_phone_list`
--
...
/*!50001 VIEW `vw_vendors_phone_list` AS select `vendors`.`vendor_name` AS `vendor_name`,`vendors`.`vendor_contact_last_name` AS `vendor_contact_last_name`,`vendors`.`vendor_contact_first_name` AS `vendor_contact_first_name`,coalesce(`vendors`.`vendor_phone`,'') AS `vendor_phone` from `vendors` where `vendors`.`vendor_id` in (select distinct `invoices`.`vendor_id` from `invoices`) */;
...
-- Dump completed on 2019-04-23 15:06:00
